<template>
    <div>
        ...
        />
        <van-field v-model="password" type="password" label="新密码" placeholder="请输入6位数密码" />
        ...
        <div class="qd">
            <van-button type="info" @click="changetransaction()">确定</van-button>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods:{
        ...
        changetransaction(){
            this.$post("user/change_pay_pwd",{token:this.token,pay_password:this.password,new_password:this.oncepassword,code:this.value}).then((res)=>{
                    console.log(res);
                    this.textes=res.msg;
                    Toast(this.textes);
                }).catch((err)=>{
